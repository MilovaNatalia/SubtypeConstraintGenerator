...
namespace SubtypeGeneratorTest
{
    ...
    public static class Tests
    {
        ...
        public static void ArrayTypes<X>()
        {
            var arr = new int[0,0];
            if ( arr is X )
            {
        ...
        {
            if (ftu is Func<M, Dictionary<T,U>> || ftkm is Func<T, Func<T,U>, T> )
            {
                number++;
            }
        }
        ...
        public static void MutualRecTest<X, Y, Z>(X x)
        where Y : Rec<Z>
        where Z: Rec<Y>
        {
            if (x is MutualRec<Y, Z>)
            {
                number++;
            }
        }
