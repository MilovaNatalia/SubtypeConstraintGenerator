...
// ------------------------------- Composition -------------------------------
    ...
    and composeStates ctx state state' =
        ...
        let constr = List.map (fillHoles ctx state) state'.traceConstraint |> List.append state.traceConstraint
        { stack = stack; heap = heap; statics = statics; frames = state.frames; pc = pc; traceConstraint = constr; typeVariables = state.typeVariables }
